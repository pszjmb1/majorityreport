...
Router.configure({
	...
	onBeforeAction: function() {
		...
		if(currentRoute) { 
			var pageClass = "page-"+ currentRoute.route.name;
			$('body').addClass(pageClass);
			Session.set('currentPage', currentRoute.route.name);
		}
	},
	onStop: function() {
		var currentRoute = Router.current();
	...
	this.route('crisisContent', {
		...
		onBeforeAction: function() {
			if(this.data()) {
				...
				var wrapperEntities = Provenance.find({mrCollectionType: {$in: ['Map', 'Timeline', 'Panel']}}).fetch();
				if(wrapperEntities) {
					//TODO: Handle panel provHadMember which is an array or objects to get the entity and attribute origin
					var subentities = _.flatten(_.pluck(wrapperEntities, "provHadMember"));
					// make sure the array contains values and not arrays or object
					// Panel contain objects in their members list
					subentities = _.flatten(_.map(subentities, function(subentity) {
						if(_.isObject(subentity)) { return _.values(subentity); }
						else { return subentity; }
					}));
